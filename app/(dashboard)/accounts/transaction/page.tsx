...
export default function TransactionPage() {
    ...
    const fetchStatement = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/accounts/${displayAccount}/statement?month=${month}`,
          {
            headers: {
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:8000/accounts/${displayAccount}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleTransaction = async (e: FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch('http://localhost:8000/accounts/transaction', {
        method: 'POST',
        headers: {
